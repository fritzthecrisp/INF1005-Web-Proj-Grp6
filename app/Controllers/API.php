...
class API extends BaseController
{
    ...
    public function getUserInstanceSets()
    {
        ...
        // check the cache 
        $cachedUserInstances = $cache->get('user_instance_sets_' . $userID);

        // if cache is empty, add cache. 
        if ($cachedUserInstances === null) {
            $db = db_connect();
            $model = new InstanceModel($db);
            $cachedUserInstances = $model->fetchUserInstances(); 

        }

        // Convert the data to JSON format
        $jsonData = json_encode($cachedUserInstances);
